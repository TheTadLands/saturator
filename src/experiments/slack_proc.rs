...
pub fn run_slack_proc_experiment(
...

    {
        ...
        writeln!(pw_file, "extra_workers,total_workers,baseline_workers,worker_id,cpu_ops_sec,io_ops_sec,sleep_ops_sec,total_ops_sec,io_errors").unwrap();
        for (total_workers, per_worker) in &per_worker_rows {
            ...
            for (wid, (wc, wi, ws, we)) in per_worker.iter().enumerate() {
                writeln!(pw_file, "{},{},{},{},{:.2},{:.2},{:.2},{:.2},{}",
                         extra, total_workers, baseline_workers, wid, wc, wi, ws, wc + wi, we).unwrap();
            }
        }
    }
