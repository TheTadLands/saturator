...
pub fn measure_thread_throughput(
    ...
    let samples: Vec<_> = (0..params.samples).map(|_| {
        measure_single_run(thread_count, io_perc, calibration, params)
    }).collect();
...
pub fn measure_single_run(
    ...
    let per_thread: Vec<[Arc<AtomicU64>; 4]> = (0..thread_count)
    ...
    for (i, pt) in per_thread.iter().enumerate() {
        ...
        let handle = std::thread::spawn(move || {
            run_saturator(i, io_perc, cpu_ops, io_ops, deadline_ns, pt_cpu, pt_io, pt_sleep, pt_errors, params, calibration);
        });
        handles.push(handle);
    }
    ...
    let cpu = cpu_ops.load(Ordering::Relaxed) as f64 / params.duration_secs as f64;
    let io = io_ops.load(Ordering::Relaxed) as f64 / params.duration_secs as f64;
...
pub fn measure_total_throughput_single(
    ...
    for (i, pt) in per_thread[..baseline_threads].iter().enumerate() {
        ...
        let handle = std::thread::spawn(move || {
            run_saturator(i, io_ratio, cpu_ops, io_ops, deadline_ns, pt_cpu, pt_io, pt_sleep, pt_errors, params, calibration);
        });
        handles.push(handle);
    }
    ...
    for (i, pt) in per_thread[baseline_threads..].iter().enumerate() {
        ...
        let handle = std::thread::spawn(move || {
            run_saturator(baseline_threads + i, io_ratio, cpu_ops, io_ops, deadline_ns, pt_cpu, pt_io, pt_sleep, pt_errors, params, calibration);
        });
        handles.push(handle);
    }
